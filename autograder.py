...
Description: Grading script for Project-2 Part-2 (FL at the Edge with IoT Greengrass)
"""
import os
import sys
import glob
...
parser = argparse.ArgumentParser(description='Project-2 Part-2 Autograder')
...
# CSV Files and Paths
grade_project           = "Project-2-Part-2"
...
print_and_log(logger, f"Num Rounds: {num_rounds}")
print_and_log(logger, "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
...
for index, row in roster_df.iterrows():
    ...
    if zip_files and os.path.isfile(zip_files[0]):
        ...
        worker_path          = "extracted/worker/worker.py"
        worker_exist         = read_and_extract_file(logger, worker_path)

        aggregator_path      = "extracted/aggregator/aggregator.py"
        aggregator_exist     = read_and_extract_file(logger, aggregator_path)

        if not cred_values or not worker_exist or not aggregator_exist:
            sanity_pass     = False
            test_pass       = False
            test_status     = "Fail"
        ...
        if sanity_pass:
            ...
            try:
                ...
                try:
                    ...
                    policy_flags = iam_obj.validate_policies()

                    if (policy_flags[0] == False):
                        print_and_log(logger, "IAMReadOnlyAccess not attached.")

                    # STEP-4: Execute test cases
                    aws_grader = grader_project2_p2(
                        logger, asuid,
                        cred_values[0], cred_values[1],
                        policy_flags[1], policy_flags[2],  # ec2, s3
                        policy_flags[3], policy_flags[0],  # lambda, iam
                        policy_flags[4], policy_flags[5],  # iot, greengrass
                        num_rounds=num_rounds)

                    test_results = aws_grader.main()
                    grade_points = test_results["grade_points"]

                    for tc_key in ["tc_1", "tc_2"]:
                    ...
                    results = append_grade_remarks(results, name, asuid,
                                            ...
                                            test_results["tc_2"][0], test_results["tc_2"][1],
                                            grade_points, grade_comments)
