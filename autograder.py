...
for index, row in roster_df.iterrows():
    ...
    if zip_files and os.path.isfile(zip_files[0]):
        ...
        test_pass, test_status, test_err, test_comments, test_script_err, test_results  = check_zip_contents(logger, name, asuid, sanity_script, zip_file, results)
        ...
        if sanity_pass:
            ...
            directories, files   = find_source_code_path(extracted_folder, ["credentials/credentials.txt"])

            print_and_log(logger, f"Following files were found in the zip: {files}")
            ...
            try:
                ...
                try:
                    ...
                    print_and_log(logger, f"[IAM-log] Following policies are attached with IAM user:cse546-AutoGrader: {policy_names}")
                    #Validate the cloudwatch alarms
                    alarm_found = cloudwatch_obj.validate_cloudwatch(policy_names)
                    if is_none_or_empty(alarm_found) == True:
                        print_and_log_warn(logger, f"[Cloudwatch-log] CAUTION !! You do not have a Cloudwatch alarm set. Kindly refer to the Project-0 document and learn how to set a billing alarm")
