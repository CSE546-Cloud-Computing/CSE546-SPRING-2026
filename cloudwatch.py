...
class aws_cloudwatch():
    ...
    def validate_cloudwatch(self, attached_policies):
        alarm_found = ""

        '''
        if "AWSBillingReadOnlyAccess" in attached_policies:
            policy_exits = True
            self.print_and_log("[Cloudwatch-log] AWSBillingReadOnlyAccess policy attached with grading IAM")
        else:
            policy_exits = False
        '''
        ...
        if policy_exits:
            ...
            alarm_found = ""
            self.print_and_log_warn("[Cloudwatch-log] AWSBillingReadOnlyAccess policy NOT attached with grading IAM")
            return alarm_found
...
if __name__ == "__main__":
    ...
    attached_policies = aws_obj.iam_client.list_attached_user_policies(UserName='cse546-AutoGrader',MaxItems=100)
    policy_names = [policy['PolicyName'] for policy in attached_policies['AttachedPolicies']]
    aws_obj.validate_cloudwatch(policy_names)
