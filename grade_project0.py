        ...
        if policy_exits:
            try:
                ...
                if e.response['Error']['Code'] == 'UnauthorizedOperation':
                    ...
                    comments = "[EC2-log] EC2 instance validation failed due:{e}. Points:[0/33.33]"
                    self.print_and_log(self.logger, comments)
                    return 0, comments
        else:
        ...
        if policy_exits:
            try:
                ...
                if e.response['Error']['Code'] == 'AccessDenied':
                    ...
                    comments = "[SQS-log] SQS validation failed due: {e}. Points:[0/33.33]"
                    self.print_and_log(self.logger, comments)
                    return 0,comments
        else:
        ...
        if policy_exits:
            try:
                ...
                if e.response['Error']['Code'] == 'AccessDenied':
                    ...
                    comments = "[S3-log] Bucket validation failed due :{e}. Points:[0/33.33]"
                    self.print_and_log(self.logger, comments)
                    return 0, comments
        else:
