...
class grader_project1():
    ...
    def validate_dynamodb_table(self):
        ...
        try:
            ...
            available_tables = response.get('TableNames', [])
            if available_tables and self.dynamodb_table_name in available_tables:
                self.print_and_log(f"[DB-log] DynamoDB Table: {self.dynamodb_table_name} exists")
                table = self.dynamodb_resource.Table(self.dynamodb_table_name)
                scan = table.scan()
                item_count = scan['Count']
                if item_count == 1000:
                    points_deducted = 0
                    comments = f"[DB-log] DynamoDB Table: {self.dynamodb_table_name} exists with 1000 items. Points deducted:{points_deducted}"
                    self.print_and_log(comments)
                    return points_deducted, comments
                else:
                    points_deducted = 33.33
                    comments = f"[DB-log] DynamoDB Table: {self.dynamodb_table_name} exists with {item_count}/1000 items. Points deducted:{points_deducted}"
                    self.print_and_log_error(comments)
                    return points_deducted, comments
            else:
                points_deducted = 33.33
                comments = f"[DB-log] DynamoDB Table: {self.dynamodb_table_name} does not exist. Points deducted:{points_deducted}"
                self.print_and_log_error(comments)
                return points_deducted, comments
        except ClientError as e:
            points_deducted = 33.33
            comments = f"[DB-log] DynamoDB Table validation failed {e}. Points deducted:{points_deducted}"
            self.print_and_log_error(comments)
            return points_deducted, comments
    ...
    def validate_initial_states(self):
        ...
        db_pts_deducted, db_logs   = self.validate_dynamodb_table()

        total_points_deducted = ec2_pts_deducted + s3_pts_deducted + db_pts_deducted
        comments = ec2_logs
        comments += s3_logs
        comments += db_logs
