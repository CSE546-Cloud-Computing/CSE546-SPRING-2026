...
class grader_project1():
    def __init__(self, logger, asuid, access_keyId, access_key, ec2_ro_access_flag, s3_full_access_flag, sqs_full_access_flag, db_full_access_flag):
        ...
        self.sqs_resources          = self.iam_session.resource('sqs', 'us-west-2')
        self.sqs_client             = self.iam_session.client('sqs', 'us-west-2')
        self.dynamodb_resources     = self.iam_session.resource('dynamodb', 'us-west-2')
        self.logger                 = logger
    ...
    def beautify_headers(self):
        ...
        column5 = " # of items in DynamoDB Table "
    ...
    def clear_dynamodb_table(self):
        try:
            table = self.dynamodb_resources.Table(self.dynamodb_table_name)
            scan = table.scan()
            with table.batch_writer() as batch:
                for item in scan.get('Items', []):
                    key = {k['AttributeName']: item[k['AttributeName']] for k in table.key_schema}
                    batch.delete_item(Key=key)
            self.print_and_log(f"[DynamoDB-log] Table:{self.dynamodb_table_name} is now EMPTY !!")
        except Exception as e:
            self.print_and_log_warn(f"[DynamoDB-log] Failed to clear table:{self.dynamodb_table_name}: {e}")
    ...
    def validate_dynamodb_table(self):
        points_deducted                 = 0
        ddb_not_exists_pts_deduction    = 5
        ddb_not_empty_pts_deduction     = 5

        if self.db_full_access_flag == True:
            self.print_and_log("[DynamoDB-log] AmazonDynamoDBFullAccess policy attached with grading IAM")
        else:
            comments = "[DynamoDB-log] AmazonDynamoDBFullAccess policy NOT attached with grading IAM"
            self.print_and_log(comments)

        self.print_and_log("[DynamoDB-log] The expectation is that the DynamoDB table should exist and be EMPTY")
        self.print_and_log("[DynamoDB-log] - WARN: If there are items in the table, they will be deleted")
        self.print_and_log("[DynamoDB-log] ---------------------------------------------------------")
        ...
        try:
            ...
            self.print_and_log(f"[DynamoDB-log] DynamoDB Table:{self.dynamodb_table_name} exists with {item_count} item(s).")

            if item_count > 0:
                points_deducted = ddb_not_empty_pts_deduction
                self.print_and_log_warn(f"[DynamoDB-log] Table is not empty. Clearing all items...")
                scan = table.scan()
                with table.batch_writer() as batch:
                    for item in scan.get('Items', []):
                        key = {k['AttributeName']: item[k['AttributeName']] for k in table.key_schema}
            ...
            comments = f"[DynamoDB-log] DynamoDB Table:{self.dynamodb_table_name} had {item_count} item(s). Points deducted:{points_deducted}"
            self.print_and_log(f"[DynamoDB-log] Points deducted:{points_deducted}")
            return points_deducted, comments

        except ClientError as e:
            if e.response['Error']['Code'] == 'ResourceNotFoundException':
                points_deducted = ddb_not_exists_pts_deduction
                comments = f"[DynamoDB-log] Table:{self.dynamodb_table_name} does not exist.Points deducted:{points_deducted}"
                self.print_and_log_error(comments)
                return points_deducted, comments
            else:
                points_deducted = ddb_not_exists_pts_deduction
                comments = f"[DynamoDB-log] Table:{self.dynamodb_table_name} validation failed due to: {e}.Points deducted:{points_deducted}"
                self.print_and_log_error(comments)
                return points_deducted, comments
        except Exception as e:
            points_deducted = ddb_not_exists_pts_deduction
    ...
    def validate_dynamodb_count(self, num_req):
        total_ddb_count_score = 5
        ddb_item_count      = self.count_dynamodb_items()
        points_per_ddb_entry = total_ddb_count_score / num_req
        test_case_points    = ddb_item_count * points_per_ddb_entry
        test_case_points    = min(test_case_points, total_ddb_count_score)
        test_case_points    = round(test_case_points, 2)
        comments = f"[Test-Case-3-log] {ddb_item_count}/{num_req} items in DynamoDB table:{self.dynamodb_table_name}.Points:[{test_case_points}/{total_ddb_count_score}]"
        self.print_and_log(comments)
        return test_case_points, comments
    ...
    def validate_latency(self, num_req, stats, deductions):
        total_test_score = 20
        completed_requests  = stats.get("completed_requests", 0)
        ...
        if completed_requests == num_req:
            ...
            if avg_latency > 0:
                ...
                deductions = [(1.2, 0, "avg latency<1.2s"),
                        (2.1, 10, "avg latency>=1.2s and avg latency<2.1s"),
                        (3, 15, "avg latency>=2.1s and avg latency<3"),
                        (float('inf'), 20, "avg latency>4.2")]
                '''
    ...
    def validate_autoscaling(self, stop_event, as_resultq):
        ...
        try:
            ...
            while not stop_event.is_set():
                ...
                ddb_item_count   = self.count_dynamodb_items()
                data_point = (req_queue_count, resp_queue_count, num_instances, ip_obj_count, ddb_item_count)
                autoscaling_data.append(data_point)
                self.print_and_log(format_string.format(req_queue_count, resp_queue_count, num_instances, ip_obj_count, ddb_item_count))
                self.print_and_log("-" * 114)
                time.sleep(2)
    ...
    def analyze_autoscaling_results(self, autoscaling_data):
        ...
        ddb_item_counts     = [data[4] for data in autoscaling_data]
        ...
        # Check S3 input bucket pattern
        self.check_s3_pattern(self.in_bucket_name, ip_bucket_counts)

        # Log DynamoDB item count pattern
        if ddb_item_counts and max(ddb_item_counts) == self.num_req:
            self.print_and_log(f"[AS-log] DynamoDB table:{self.dynamodb_table_name} items increased from {ddb_item_counts[0]} to {max(ddb_item_counts)}.")
        else:
            self.print_and_log_error(f"[AS-log] DynamoDB table:{self.dynamodb_table_name} pattern is not as expected. Max items: {max(ddb_item_counts) if ddb_item_counts else 0}")
    ...
    def evaluate_iaas(self, num_req, ip_addr, img_folder, pred_file):
        ...
        try:
            ...
            ddb_score, ddb_log                   = self.validate_dynamodb_count(num_req)
            ...
            latency_rubrics = [(1.2, 0, "avg latency<1.2s"), (2.1, 10, "avg latency>=1.2s and avg latency<2.1s"),
                    (3, 15, "avg latency>=2.1s and avg latency<3"), (float('inf'), 20, "avg latency>4.2")]

            latency_score, latency_log           = self.validate_latency(num_req, stats, latency_rubrics)
            self.print_and_log("[Test-Case-3-log] ---------------------------------------------------------")

            test_case_points += scale_in_lat_score + in_s3_score + ddb_score + correctness_score + latency_score
    ...
    def evaluate_iaas_warm_cache(self, num_req, ip_addr, img_folder, pred_file):

        self.num_req = num_req

        # Empty S3 buckets and SQS queues before warm cache run, but keep DynamoDB intact
        self.print_and_log("[Test-Case-WC-log] Preparing for warm cache run ...")
        self.print_and_log("[Test-Case-WC-log] Emptying S3 buckets and SQS queues (DynamoDB cache retained)")
        try:
            self.empty_s3_bucket(self.in_bucket_name)
        except Exception as e:
            self.print_and_log_warn(f"[Test-Case-WC-log] S3 cleanup warning: {e}")
        ...
        try:
            ...
            if ip_queue_requests:
                self.print_and_log_warn(f"[Test-Case-WC-log] Purging Request SQS. Waiting 60 seconds ...")
                self.sqs_client.purge_queue(QueueUrl=self.req_sqs_name)
                time.sleep(60)
            if op_queue_response:
                self.print_and_log_warn(f"[Test-Case-WC-log] Purging Response SQS. Waiting 60 seconds ...")
                self.sqs_client.purge_queue(QueueUrl=self.resp_sqs_name)
                time.sleep(60)
        except Exception as e:
            self.print_and_log_warn(f"[Test-Case-WC-log] SQS cleanup warning: {e}")

        # Wait for app-tier instances to fully scale in from previous test
        self.print_and_log("[Test-Case-WC-log] Waiting for app-tier instances to scale in before warm cache run ...")
        ...
        try:
            ...
            self.print_and_log_error(f"[Test-Case-WC-log] Workload generator failed with return code {e}")
            self.print_and_log_error(f"[Test-Case-WC-log] Standard output: {e.stdout}")
            self.print_and_log_error(f"[Test-Case-WC-log] Standard error: {e.stderr}")
            comments = f"[Test-Case-WC-log] Error: {e.stdout} + {e.stderr}"
            return 0, comments

        latency_rubrics = [(0.6, 0, "avg latency<0.6s"), (1.0, 10, "avg latency>=0.6s and avg latency<1.0s"),
                (1.5, 15, "avg latency>=1.0s and avg latency<1.5"), (float('inf'), 20, "avg latency>1.5")]

        latency_score, latency_log = self.validate_latency(num_req, stats, latency_rubrics)
        self.print_and_log("[Test-Case-WC-log] ---------------------------------------------------------")
    ...
    def validate_initial_states(self):
        ...
        ddb_pts_deducted, ddb_logs = self.validate_dynamodb_table()

        total_points_deducted = ec2_pts_deducted + s3_pts_deducted + sqs_pts_deducted + ddb_pts_deducted
        comments = ec2_logs
        comments += s3_logs
        comments += sqs_logs
        comments += ddb_logs
    ...
    def main(self, num_req, ip_addr, img_folder, pred_file):
        ...
        self.print_and_log("----------------- Executing Test-Case:3 (Warm Cache) ----------------")
        test_results["tc_4"] = self.evaluate_iaas_warm_cache(num_req, ip_addr, img_folder, pred_file)
