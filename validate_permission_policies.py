...
class iam_policies():
    ...
    def validate_dynamodb(self, attached_policies):

        if "AmazonDynamoDBFullAccess" in attached_policies:
            policy_exits = True
            self.print_and_log(self.logger, "[IAM-log] AmazonDynamoDBFullAccess policy attached with grading IAM")
        else:
            policy_exits = False
            self.print_and_log(self.logger, "[IAM-log] AmazonDynamoDBFullAccess policy NOT attached with grading IAM")
    ...
    def validate_policies(self):
        ...
        try:
            ...
            db_full_access_flag  = self.validate_dynamodb(policy_names)
            return iam_ro_access_flag, ec2_ro_access_flag, s3_full_access_flag, sqs_full_access_flag, db_full_access_flag
