...
class iam_policies():
    ...
    def validate_dynamo_db(self, attached_policies):

        if "AmazonDynamoDBReadOnlyAccess" in attached_policies:
            policy_exits = True
            self.print_and_log(self.logger, "[IAM-log] AmazonDynamoDBReadOnlyAccess policy attached with grading IAM")
        else:
            policy_exits = False
            self.print_and_log_error(self.logger, "[IAM-log] AmazonDynamoDBReadOnlyAccess policy NOT attached with grading IAM")
        return policy_exits
    ...
    def validate_policies(self):
        ...
        try:
            ...
            db_ro_access_flag   = self.validate_dynamo_db(policy_names)
            return iam_ro_access_flag, ec2_ro_access_flag, s3_full_access_flag, db_ro_access_flag

        except ClientError as e:
            iam_ro_access_flag   = False
            ec2_ro_access_flag   = None
            s3_full_access_flag  = None
            db_ro_access_flag    = None
